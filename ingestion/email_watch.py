...
`kind='email'`, `status='active'`), this polls the mailbox for new
(unread) mail, drops auto-responders / list mail / the mailbox's own
messages, and enqueues one `run_flow` job per real message against the
tenant's published flow for the channel's team. The job is keyed on the
message's `Message-ID`, so a redelivered message never double-enqueues and
the poll window can safely overlap between runs -- no watermark to persist.
Processed messages are marked read (IMAP `\\Seen` / Gmail `UNREAD` removed),
never deleted.
...
def tick(*, tenant: str | None, lookback_days: int, limit: int, dry_run: bool) -> int:
    sb = get_supabase()
