...
Also resolved: **Google OAuth login status** was flagged as unverifiable
via the Supabase MCP tools (`get_project` doesn't expose auth provider
config) — queried `auth.identities` directly instead (`execute_sql`) and
found 2 real `provider='google'` identities, confirming the "Continue
with Google" button (row 18d) actually works live; that row's stale
"needs the dashboard steps done" note was wrong and has been corrected.
