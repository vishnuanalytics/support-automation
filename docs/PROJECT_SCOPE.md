...
done, 20c–20d next.** Migrations `001`–`035` applied (`034`/`035` =
Phase 20a: `tenant_integrations` poller columns + the Supabase-Vault
`integration_secret_*` RPCs). 113 offline pytest tests + web tsc/vitest
...
**2026-08-30 — Phase 20a + 20b done.** 20a: `interpreter/mailbox.py` +
`GET/PUT/DELETE /api/integrations/email`, `POST …/test`,
`GET …/google/{authorize,callback}` — a per-tenant mailbox config
(IMAP/app-password or Gmail OAuth) entered by a workspace **owner**; the
password / refresh token goes to **Supabase Vault** (migration `035`),
never to the browser. 20b: `ingestion/email_watch.py --once` polls each
active channel, drops auto-responders / own-mail, and enqueues a
`run_flow` job keyed on `Message-ID` against the tenant's published flow;
messages are marked read, never deleted; a fetch error → `status='error'`.
Next: **20c** outbound send behind the hard guard (`outcome.action ==
"auto_reply"` + the per-channel `auto_send_enabled` switch), then **20d**
the web Channels panel + `docs/EMAIL_SETUP.md`. Gmail OAuth needs
`GOOGLE_CLIENT_ID`/`SECRET` in `.env` + the `.../email/google/callback`
redirect registered (operator step; IMAP path is fully verified).
