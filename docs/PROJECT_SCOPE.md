...
1. **Live-verify the connectors** (all need creds this environment
   ...
   - Phase 14 Globex billing branch end-to-end once Groq daily quota
     resets.
2. Polish: Phase 16 `when`/`then` form builder (currently JSON
   textareas); Phase 14 file upload (`.pdf`/`.docx`); an `eval/e2e`
   case that only passes when an internal KB entry is consulted.
