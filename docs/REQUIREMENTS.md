...
| **FR-45** Generic run context (**P5a+P5b built 2026-09-02; P5c–d planned**) | The run state is `CaseState` — every node handler, the trace, persistence, and `builder._context` assume a Salesforce support Case. A generic **`RunContext`** MUST be introduced alongside it, with `sf_case` as one adapter that produces it, threaded through `builder` / `registry` / `runs` / `trace`. This is the prerequisite for FR-46…FR-48. |
| **FR-46** Triggers (**P5b partial — webhook + authed endpoint built 2026-09-02; token/schedule in P6**) | A flow MUST be startable by a hosted **webhook** (`POST /t/{flow_token}` → enqueue a run with the JSON body as the `RunContext`) and by a **schedule**, decoupled from Case/CDC/email plumbing. |
