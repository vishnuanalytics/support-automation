...
| **FR-25** Salesforce → automation push (CDC, Phase 20l) | `ingestion.sf_cdc_watch` — a long-lived gRPC client on the **Pub/Sub API** streaming `Case` + `EmailMessage` Change Data Capture. Enqueues a `run_flow` job for a new Case, a new **inbound email on an existing Case**, and a **queue (OwnerId) change**. Durable: last `replay_id` per topic persisted in `sf_cdc_state` (migration `043`) → restart resumes (72h retention). Shares `interpreter.sf_ingest.enqueue_case_run` with FR-24. **Code-complete, not live-verified** — needs the process running + a JWT session (`Case`/`EmailMessage` already in Setup → Change Data Capture). docker-compose `cdc` service. |
...
| FR-25 | migration `043` not yet applied; subscriber not yet run against the org (no live gRPC verification). No SF-side config needed beyond the CDC entity selection, which is done. |
