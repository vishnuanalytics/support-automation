...
| **FR-47** Declarative connectors + connections manager (**P6c backend built 2026-09-02; web panel + `transform` node pending**) | A connector MUST be data, not a hardcoded Python node handler — a minimal spec (`{auth: oauth2\|apikey, base_url, actions:[…]}`), plus a per-tenant **connections** UI (today only Slack/Google have a Connect button). |
