...
| **FR-45** Generic run context (**P5a built 2026-09-02; P5b–d planned**) | The run state is `CaseState` — every node handler, the trace, persistence, and `builder._context` assume a Salesforce support Case. A generic **`RunContext`** MUST be introduced alongside it, with `sf_case` as one adapter that produces it, threaded through `builder` / `registry` / `runs` / `trace`. This is the prerequisite for FR-46…FR-48. |
