...
def test_assert_not_locked_raises_for_a_canceled_tenant_even_with_byok(monkeypatch):
    """2026-09-20: a canceled subscription is blocked unconditionally, same
    as locked -- BYOK only ever covered the LLM call, never the platform
    fee a canceled tenant has stopped paying."""
    from interpreter import billing, llm

    monkeypatch.setattr(llm, "tenant_has_byok", lambda tid: True)
    sb = _BillingSB(tenants=[{"tenant_id": "t1", "billing_status": "canceled"}])
    with pytest.raises(billing.BillingLockedError):
        billing.assert_not_locked("t1", sb)
...
def test_assert_not_locked_allows_every_other_status(monkeypatch):
    ...
    monkeypatch.setattr(llm, "tenant_has_byok", lambda tid: True)
    for status in ("trialing", "active", "grace"):
